...
int setDisplays(int argc, char* argv[]) {
...
const char* currentProfile() {
    ...
    static char rtrnstrng[2046] = "";
    memset(rtrnstrng, 0, sizeof(rtrnstrng));
    ...
    for (int i = 0; i < screenCount; i++) {
        ...
        if (curMode.derived.freq) {
            snprintf(hz, sizeof(hz), "h:%i ", curMode.derived.freq);
        }
        ...
        snprintf(dsplystrng, sizeof(dsplystrng), "i:%s%s r:%ix%i %sc:%i s:%s o:%i,%i d:%i\n", curScreenUUID, mirrors, (int) CGDisplayPixelsWide(curScreen.id), (int) CGDisplayPixelsHigh(curScreen.id), hz, curMode.derived.depth, scaling, (int) CGDisplayBounds(curScreen.id).origin.x, (int) CGDisplayBounds(curScreen.id).origin.y, (int) CGDisplayRotation(curScreen.id));
        
        strcat(rtrnstrng, dsplystrng);
    }
    printf("\n");
    return rtrnstrng;
}
